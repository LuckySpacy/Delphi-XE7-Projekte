...
type
  TAktieDblClickEvent=procedure(Sender: TObject; aAK_Id: Integer; aWKN, aAktie: string) of object;
...
type
  ...
  private
    ...
    fCol: RCol;
    fBI_Id: Integer;
    fOnAktieDblClick: TAktieDblClickEvent;
  ...
  public
    ...
    procedure Aktual;
    procedure LeseDaten;
    property OnAktieDblClick:TAktieDblClickEvent read fOnAktieDblClick write fOnAktieDblClick;
  end;
...
begin
  ...
  grd.Cells[fCol.PJHoch,0] := 'P. JHoch';
  grd.Cells[fCol.PHJTief,0] := 'P. HJHoch';
  grd.Cells[fCol.PSpanne,0] := 'P. Spanne';
  ...
  grd.ColWidths[fCol.PJHoch] := 80;
  grd.ColWidths[fCol.PHJTief] := 80;
  grd.ColWidths[fCol.PSpanne] := 80;
...
begin
  ...
  begin
    ...
    grd.Cells[fCol.PJHOCH, iRow] := Ansicht.FieldByName('PJHOCH').AsString;
    grd.Cells[fCol.PHJTIEF, iRow] := Ansicht.FieldByName('PHJTIEF').AsString;
    grd.Cells[fCol.PSPANNE, iRow] := Ansicht.FieldByName('PSPANNE').AsString;
  end;
...
procedure Tfra_Ansicht.grdDblClickCell(Sender: TObject; ARow, ACol: Integer);
var
  Ansicht: TMySqlTSIAnsicht;
begin
  ...
  if Assigned(fOnAktieDblClick) then
    fOnAktieDblClick(Self, Ansicht.FieldByName('AK_ID').AsInteger, Ansicht.FieldByName('WKN').AsString, Ansicht.FieldByName('Aktie').AsString);
end;
